...
public final class StandardVideoStatsDataProvider implements VideoStatsDataProvider {
	...
	public Void save(final String id, final VideoStats stats) {
		...
		Entity task = Entity.newBuilder(key).set("timesPlayed", stats.timesPlayed)
				.set("timesFinished", stats.timesFinished).set("videoId", id)
				.set("timesMarkedOffVocal", stats.timesMarkedOffVocal)
				.set("timesMarkedWithVocal", stats.timesMarkedWithVocal).build();
		datastore.put(task);
		return null;
	}

	private Map<String, VideoStats> convertTasks(final Iterator<Entity> entities) {
